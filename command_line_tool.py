...
#------
    def evaluate_listing_string(self,initialWorkdir,builder: Builder,classic_dirent,classic_listing,debug)->List[CWLObjectType]:
        ...
        return cast(List[CWLObjectType], ls_evaluated)
#------
    def evaluate_listing_expr_or_dirent(self,initialWorkdir,builder: Builder,classic_dirent,classic_listing,debug)->List[CWLObjectType]:
...
#------
    def check_output_items(self,initialWorkdir:CWLObjectType,ls: List[CWLObjectType],debug:bool):
...
#------
    def check_output_items2(self,initialWorkdir:CWLObjectType,ls: List[CWLObjectType],cwl_version:str,debug:bool):
...
#------
    def setup_output_items(self,initialWorkdir:CWLObjectType, builder:Builder, ls: List[CWLObjectType],debug:bool):
    ...
    def _initialworkdir(self, j: JobBase, builder: Builder) -> None:
        ...
        if isinstance(initialWorkdir["listing"], str):
            ls = self.evaluate_listing_string(initialWorkdir,builder,classic_dirent,classic_listing,debug)
        else:
            ls = self.evaluate_listing_expr_or_dirent(initialWorkdir,builder,classic_dirent,classic_listing,debug)

        self.check_output_items(initialWorkdir, ls, debug)

        self.check_output_items2(initialWorkdir, ls, cwl_version, debug)

        j.generatefiles["listing"] = ls
        self.setup_output_items(initialWorkdir, ls, builder, debug)
#------
...
#------
    def handle_tool_time_limit(self, builder:Builder, j:JobBase, debug:bool):
    ...
    def handle_network_access(self,builder:Builder,j : JobBase,debug:bool):
        networkaccess, _ = self.get_requirement("NetworkAccess")
        if networkaccess is None:
            return
...
#------
    def handle_env_var(self, builder:Builder, debug:bool):
    ...
    def setup_command_line(self,builder:Builder, j:JobBase, debug:bool):
    ...
    def handle_mpi_require(self,runtimeContext:RuntimeContext,builder:Builder,j:JobBase, debug:bool):
        ...
        j.mpi_procs = np
#------
    def setup_std_io(self, builder:Builder, j:JobBase, reffiles:List[CWLObjectType], debug:bool):
...
#------
    def handle_mutation_manager(self,builder:Builder,j:JobBase):
        readers = {}  # type: Dict[str, CWLObjectType]
        muts = set()  # type: Set[str]

        if builder.mutation_manager is None:
            return readers
...
#------
    def job(
        ...
        if runtimeContext.cachedir and enableReuse:
            output_callbacks = self.cache_context()
            if output_callbacks:
                return
        builder = self._init_job(job_order, runtimeContext)
        ...
        self.setup_std_io(builder,j,reffiles, debug)
        ...
        readers = self.handle_mutation_manager(builder, j)
        self.handle_tool_time_limit(builder,j,debug)

        self.handle_network_access(builder,j,debug)

        # Build a mapping to hold any EnvVarRequirement
        required_env = self.handle_env_var(builder,debug)
        # Construct the env
        j.prepare_environment(runtimeContext, required_env)

        self.setup_command_line(builder, j, debug)
        ...
        self.handle_mpi_require(runtimeContext,builder,j ,debug)
        yield j
#------
...
#------
    def glob_output(self,builder:Builder,binding,debug:bool,outdir: str,
        fs_access: StdFsAccess,
        compute_checksum: bool,revmap)->Union[List[CWLOutputType],List[str]]:
        r = []  # type: List[CWLOutputType]
        globpatterns = []  # type: List[str]
        if "glob" not in binding:
        ...
        return r,globpatterns
#------
    def collect_secondary_files(self,schema: CWLObjectType,builder:Builder,result: Optional[CWLOutputType],debug:bool,
        fs_access: StdFsAccess,revmap):
                    ...
                    for sf in aslist(schema["secondaryFiles"]):
                        ...
                        for sfitem in aslist(sfpath):
                            ...
                            if fs_access.isfile(sfitem["location"]):
                                ...
                                primary["secondaryFiles"].append(sfitem)
#------
    def handle_output_format(self, schema:CWLObjectType,builder: Builder, result:CWLOutputType, debug:bool):
        if "format" in schema:
...
#------
    def collect_output(
        ...
        if "outputBinding" in schema:
            ...
            r, globpatterns = self.glob_output(builder,binding,debug,outdir,fs_access,compute_checksum,revmap)
            ...
            if "secondaryFiles" in schema:
                self.collect_secondary_files(schema,builder,result,debug,fs_access,revmap)

            self.handle_output_format(schema,builder, result, debug)
