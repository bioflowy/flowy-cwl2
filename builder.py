...
#------
    def handle_union(self,
                     ...
                     value_from_expression: bool,
                     lead_pos: Optional[Union[int, List[int]]] = None,
                     tail_pos: Optional[Union[str, List[int]]] = None) -> List[MutableMapping[str, Union[str, List[int]]]] :
        bound_input = False
...
#------
    def bind_input(
        ...
        if "inputBinding" in schema and isinstance(schema["inputBinding"], MutableMapping):
            binding, value_from_expression = self.handle_binding(schema, datum, lead_pos, tail_pos, debug)

        # Handle union types
        if isinstance(schema["type"], MutableSequence):
            if not value_from_expression:
                return self.handle_union(schema, datum, discover_secondaryFiles, value_from_expression, lead_pos, tail_pos)
            else:
                self.handle_union(schema, datum, discover_secondaryFiles, value_from_expression, lead_pos, tail_pos)
        elif isinstance(schema["type"], MutableMapping):
            self.handle_map(schema, datum, discover_secondaryFiles, lead_pos, tail_pos, bindings, binding, value_from_expression)
        else:
            if schema["type"] == "org.w3id.cwl.salad.Any":
                self.handle_any(schema, datum)

            if schema["type"] in self.schemaDefs:
                schema = self.schemaDefs[cast(str, schema["type"])]

            if schema["type"] == "record":
                datum = self.handle_record(schema, datum, discover_secondaryFiles, lead_pos, tail_pos, bindings)

            if schema["type"] == "array":
                binding = self.handle_array(schema, datum, discover_secondaryFiles, lead_pos, tail_pos, bindings, binding)
            ...
            if schema["type"] == "org.w3id.cwl.cwl.File":
                self.handle_file(schema, datum, discover_secondaryFiles, debug, binding)

            if schema["type"] == "org.w3id.cwl.cwl.Directory":
                datum = self.handle_directory(schema, datum)
        ...
        return bindings
#------
    def handle_any(self, schema:CWLObjectType, datum:Union[CWLObjectType, List[CWLObjectType]]):
    ...
    def handle_directory(self,  schema:CWLObjectType, datum:Union[CWLObjectType, List[CWLObjectType]]):
...
#------
    def handle_file(self,  
        ...
        if "secondaryFiles" in schema:
            return self.handle_secondary_file(schema, datum, discover_secondaryFiles, debug)

        if "format" in schema:
            self.handle_file_format(schema, datum, debug)
...
#------
    def handle_secondary_file(self, 
                              schema: CWLObjectType,
                              datum: Union[CWLObjectType, List[CWLObjectType]],
                              discover_secondaryFiles: bool,
                              debug:bool):
        ...
        for num, sf_entry in enumerate(sf_schema):
            ...
            for sfname in aslist(sfpath):
                if not sfname:
                    continue
                self.handle_secondary_path(schema, datum, discover_secondaryFiles, debug, sf_entry, sf_required, sfname)

        normalizeFilesDirs(
                        cast(MutableSequence[CWLObjectType], datum["secondaryFiles"])
                    )
#------
    def handle_secondary_path(self, 
                              schema: CWLObjectType,
                              datum: Union[CWLObjectType, List[CWLObjectType]],
                              discover_secondaryFiles: bool,
                              debug: bool,
...
#------
    def handle_array(self, 
        ...
        return binding
#------
    def handle_record(self, schema, datum, discover_secondaryFiles, lead_pos, tail_pos, bindings):
        datum = cast(CWLObjectType, datum)
        ...
        return datum
#------
    def handle_map(self, schema, datum, discover_secondaryFiles, lead_pos, tail_pos, bindings, binding, value_from_expression):
...
#------
    def handle_binding(self, schema, datum, lead_pos, tail_pos, debug):
